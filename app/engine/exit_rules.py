...
Precedence matters and is deliberate:
    ...
    2. trailing stop    lock in what the move gave you
    3. take profit      then take the win
    4. time stop        then stop paying theta for nothing
    5. session flatten  then respect the venue's clock
"""
...
def evaluate(
    ...
    *,
    ...
    # --- 5. Session flatten (equities only; crypto has no close)
    ...
    # --- 6. Stale data. Not a close signal by itself, but worth surfacing.
    ...
    return ExitSignal.hold(f"{pnl_pct:+.1f}% | target {plan.target_price:.4f} "
